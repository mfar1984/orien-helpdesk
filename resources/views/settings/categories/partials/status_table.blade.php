<x-ui.data-table
    :headers="[
        ...
        ['label' => 'Default', 'align' => 'text-center'],
        ['label' => 'Closed', 'align' => 'text-center'],
...
>
    ...
    <tr class="hover:bg-gray-50">
        ...
        <td class="px-6 py-4 whitespace-nowrap text-center">
            @if($item->is_default)
            <span class="inline-flex px-2 py-1 text-xs font-medium rounded bg-blue-100 text-blue-600" style="font-size: 10px;">Default</span>
            @else
        ...
        <td class="px-6 py-4 whitespace-nowrap text-center">
            @if($item->is_closed)
            <span class="inline-flex px-2 py-1 text-xs font-medium rounded bg-gray-100 text-gray-600" style="font-size: 10px;">Closed</span>
            @else
            <span class="text-xs text-gray-400">-</span>
            @endif
        </td>
        ...
        <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
            <x-ui.action-buttons
                ...
                :delete-onclick="($canDelete ?? false) && !$item->is_default ? 'showDeleteModal(\'' . route('settings.categories.status.destroy', $item) . '\')' : null"
            />
        </td>
    </tr>
    @empty
