...
<div id="item-modal" class="fixed inset-0 flex items-center justify-center" style="background-color: rgba(0,0,0,0.5) !important; z-index: 9999 !important; display: none;">
    <div style="background-color: #ffffff !important; border-radius: 12px !important; box-shadow: 0 25px 50px -12px rgba(0,0,0,0.25) !important; width: 100% !important; max-width: 520px !important; margin: 16px !important; overflow: hidden !important;">
        <div style="padding: 16px 20px !important; border-bottom: 1px solid #e5e7eb !important; display: flex !important; align-items: center !important; justify-content: space-between !important; background-color: #f9fafb !important;">
            <div style="display: flex !important; align-items: center !important; gap: 10px !important;">
                <div style="width: 36px !important; height: 36px !important; border-radius: 8px !important; background-color: #3b82f6 !important; display: flex !important; align-items: center !important; justify-content: center !important;">
                    <span class="material-symbols-outlined" style="font-size: 20px !important; color: #ffffff !important;">flag</span>
                </div>
                <h3 id="modal-title" style="font-size: 14px !important; font-weight: 600 !important; color: #111827 !important; font-family: Poppins, sans-serif !important; margin: 0 !important;">Create Priority</h3>
            </div>
        ...
        <form id="item-form" method="POST" action="{{ route('settings.categories.priority.store') }}">
            @csrf
            <input type="hidden" name="_method" id="form-method" value="POST">
            <input type="hidden" name="icon" id="input-icon" value="flag">
            <div style="padding: 20px !important; max-height: 65vh !important; overflow-y: auto !important;">
                <div style="display: flex !important; flex-direction: column !important; gap: 16px !important;">
                    <div>
                        ...
                        <input type="text" name="name" id="input-name" required style="width: 100% !important; padding: 10px 12px !important; border: 1px solid #d1d5db !important; border-radius: 6px !important; font-family: Poppins, sans-serif !important; font-size: 12px !important; color: #1f2937 !important; outline: none !important; box-sizing: border-box !important;" placeholder="Priority name">
                    </div>
                    <div>
                        ...
                        <textarea name="description" id="input-description" rows="2" style="width: 100% !important; padding: 10px 12px !important; border: 1px solid #d1d5db !important; border-radius: 6px !important; font-family: Poppins, sans-serif !important; font-size: 12px !important; color: #1f2937 !important; resize: vertical !important; outline: none !important; box-sizing: border-box !important;" placeholder="Priority description"></textarea>
                    </div>
                    <!-- Icon Picker Grid -->
                    <div>
                        ...
                        <div style="border: 1px solid #d1d5db !important; border-radius: 6px !important; padding: 12px !important;">
                            ...
                            <div style="margin-top: 8px !important; padding-top: 8px !important; border-top: 1px solid #e5e7eb !important;">
                                <span style="font-size: 10px !important; color: #6b7280 !important; font-family: Poppins, sans-serif !important;">Selected: <span id="selected-icon-name" style="font-weight: 500 !important; color: #374151 !important;">flag</span></span>
                            </div>
                        </div>
                    </div>
...
@push('scripts')
<script>
const priorityIcons = ['flag', 'priority_high', 'low_priority', 'warning', 'error', 'info', 'bolt', 'speed', 'timer', 'schedule', 'alarm', 'notification_important', 'report', 'report_problem', 'emergency', 'crisis_alert', 'trending_up', 'trending_down', 'arrow_upward', 'arrow_downward', 'keyboard_double_arrow_up', 'keyboard_double_arrow_down', 'star', 'grade', 'whatshot', 'local_fire_department', 'electric_bolt', 'flash_on'];

function initIconGrid() {
    const grid = document.getElementById('icon-grid');
    grid.innerHTML = priorityIcons.map(icon => `
...
function showCreateModal() {
    document.getElementById('modal-title').textContent = 'Create Priority';
    document.getElementById('item-form').action = '{{ route('settings.categories.priority.store') }}';
...
function showEditModal(item) {
    document.getElementById('modal-title').textContent = 'Edit Priority';
    document.getElementById('item-form').action = '{{ url('settings/categories/priority') }}/' + item.id;
    document.getElementById('form-method').value = 'PUT';
    document.getElementById('input-name').value = item.name;
    document.getElementById('input-description').value = item.description || '';
    selectIcon(item.icon || 'flag');
